...
params = [
    ...
    'num_epochs','class_weight', 'roundup'
]
...
for k, (train, test) in enumerate(outer_cv):    
    ...
    for l, (l_train, l_test) in enumerate(inner_cv):
        inner_df = pd.read_csv(f'cv_3/cv_results_multi_impact_all_{k}_{l}.csv')
    ...
    best_model = (inner_scores
                  .groupby(params)['F1 macro']
                  .mean()
                  .sort_values(ascending=False)
                  .reset_index() 
                 ).to_dict('records')[0]

    del best_model['F1 macro']
    print(best_model)
    ...
    outer_scores['rank_k'] = k
    pd.DataFrame.from_dict([outer_scores]).to_csv(f'cv_3/outer_cv_impacts_all_{k}.csv',index=False)
    
    np.save(f"cv_3/y_preds_impacts_{k}.npz",y_preds)

...
for k in range(3):
    inner_df = pd.read_csv(f'cv_3/model_selection_impact_{k}.csv')
...
for k, (train, test) in enumerate(outer_cv.split(seen_index)):    
    ...
    np.save(f"cv_3/y_preds_5fold_impacts_{k}.npz",y_preds)

np.save("cv_3/impact_pred_ids",df.loc[unseen_index,"id"])

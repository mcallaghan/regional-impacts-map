...
for k, (train, test) in enumerate(outer_cv):    
    ...
    for l, (l_train, l_test) in enumerate(inner_cv):
        inner_df = pd.read_csv(f'cv_3/cv_results_multi_impact_{k}_{l}.csv')
    ...
    pd.DataFrame.from_dict([outer_scores]).to_csv(f'cv_3/outer_cv_impacts_noround_{k}.csv',index=False)
    
    np.save(f"cv_3/y_preds_outer__impacts_{k}.npz",y_preds)

...
for k, (train, test) in enumerate(outer_cv):    
    ...
    for l, (l_train, l_test) in enumerate(inner_cv):
        ...
        try:
            pr = param_space[0]
            cv_results=pd.read_csv(f'cv_3/cv_results_multi_impact_{rank_i}_{rank_j}.csv').to_dict('records')
            params_tested=pd.read_csv(f'cv_3/cv_results_multi_impact_{rank_i}_{rank_j}.csv')[list(pr.keys())].to_dict('records')
        except:
            cv_results = []
            params_tested = []
        for pr in param_space:
            ...
            pd.DataFrame.from_dict(cv_results).to_csv(f'cv_3/cv_results_multi_impact_{rank_i}_{rank_j}.csv',index=False)
            gc.collect()
